...
const DEFAULT_LIFE_MARKDOWN_BYTES = 32 * 1024;
...
export function lifeProjectionText(state: LifeState): string {
  const chunks = [
    "# Life OS continuity",
    "",
    "> Generated read-only projection from private Life OS state.",
    "> Update via `/memory life ...` or Life OS tools; do not edit this file directly.",
    "",
    `Updated: ${state.updatedAt}`,
    "",
    formatLifeProfile(state),
    "",
    formatLifeGoals(state, Math.max(state.goals.length, 1)),
    "",
    formatLifeReflections(state, Math.max(state.reflections.length, 1)),
  ];
  return truncateUtf8(chunks.join("\n"), DEFAULT_LIFE_MARKDOWN_BYTES);
}
