...
import { clearRemoteTurnOrigin, setRemoteTurnOrigin } from "@nazar/core/remote-origin";
import { hasInteractiveUi, notify } from "@nazar/core/shared";
import { getTranscriber } from "@nazar/core/transcriber";
import {
  assistantText,
  deleteWhatsAppAuth,
  extractText,
...
function whatsappDependencyError(error: unknown): Error {
  ...
  return new Error([
    ...
    "Repo-local setup: run `npm install --workspace @nazar/whatsapp` from the Nazar checkout.",
    "Installed package setup: install optional WhatsApp dependencies for the package or reinstall `@nazar/whatsapp` with optional dependencies enabled.",
  ].join("\n"));
}
...
async function handleIncoming(pi: ExtensionAPI, baileys: BaileysRuntime, payload: any): Promise<void> {
  for (const message of payload?.messages ?? []) {
    ...
    try {
      ...
      if (audioMessage) {
        const transcribe = getTranscriber();
        if (!transcribe) {
          await socket?.sendMessage(filter.jid, { text: "Voice transcription is not available in this Nazar install. Install/enable @nazar/voice to process audio messages." });
          await markReadBestEffort(message);
          continue;
        }
        ...
        const transcript = (await transcribe(pcm)).trim();
        if (!transcript) {
