...
def build_all():
    ...
    for kind, iris in TOOL_IRIS.items():
        compose_tool_eye(iris).save(os.path.join(tools_dir, f"{kind}.png"), "PNG", optimize=True)
    print(f"  composed {len(TOOL_IRIS)} tool eyes")
