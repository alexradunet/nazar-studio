...
const SHEETS = [
  ...["mage-alien", "nazar", "nazar-expr", "soul"].map((name) => ({
  ...
  ...[
    // Shared "eye" sprites — Nazar's eye showing each tool. Many tool kinds map
    // to one eye (see KIND_TO_EYE in lib/ui/pixel-avatar.ts), so the tool sheets
    // are deduped to these.
    "eye-read", "eye-write", "eye-edit", "eye-search", "eye-bash", "eye-files",
