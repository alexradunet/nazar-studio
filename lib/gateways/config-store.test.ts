...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "balaur-gw-"));
...
describe("config store", () => {
  ...
  test("save merges across calls and persists", () => {
    saveStoredConfig({ gateway: "fake", owner: "+40712345678" }, path);
    saveStoredConfig({ mirrorLocal: true }, path);
    expect(loadStoredConfig(path)).toEqual({ gateway: "fake", owner: "+40712345678", mirrorLocal: true });
  });
...
describe("resolveEffectiveConfig", () => {
  test("stored wins over env; configured when gateway+owner set", () => {
    saveStoredConfig({ gateway: "fake", owner: "+40712345678", toolPings: true }, path);
    const eff = resolveEffectiveConfig(
      { BALAUR_GATEWAY: "other", BALAUR_GATEWAY_OWNER: "+1", BALAUR_GATEWAY_TOOL_PINGS: "0" },
      path,
    );
  ...
  test("falls back to env when nothing is stored", () => {
    const eff = resolveEffectiveConfig({ BALAUR_GATEWAY: "fake", BALAUR_GATEWAY_OWNER: "+1" }, path);
