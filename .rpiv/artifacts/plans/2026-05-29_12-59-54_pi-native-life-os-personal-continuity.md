---
date: 2026-05-29T12:59:54+0300
author: Alex Radu
commit: 19034189
branch: main
repository: nazar
topic: "Pi-native Life OS personal continuity"
tags: [plan, memory, life-os, personal-continuity]
status: ready
parent: ".rpiv/artifacts/designs/2026-05-29_11-55-28_pi-native-life-os-personal-continuity.md"
last_updated: 2026-05-29T13:12:14+0300
last_updated_by: Alex Radu
last_updated_note: "Applied Step 4 plan-review fixes"
---

# Pi-native Life OS Personal Continuity Implementation Plan

## Overview

This plan implements the ready design artifact `.rpiv/artifacts/designs/2026-05-29_11-55-28_pi-native-life-os-personal-continuity.md`. It adds a Pi-native personal-continuity MVP inside `@nazar/memory`: private profile/goals/reflections state under the existing memory `STATE_DIR`, explicit `/memory life ...` commands, focused Life OS model tools, and tests/docs while leaving default durable prompt injection unchanged.

Inheriting 3 slices from the design as 3 phases (1:1):

```txt
1. Life state and readout foundation - private state/readout foundation (3 files)
2. `/memory life ...` commands - `/memory life ...` command surface (3 files)
3. Focused Life OS tools and docs - focused model tools, docs, and terminal validation (4 files)
```

Parallelism per design ordering: Phase 1 must land first. Phases 2 and 3 both depend on the state/readout foundation; source implementation should run sequentially. Total: 8 files across 3 phases. Success Criteria pass through from the design `## Slices` section unchanged.
...
### Overview
Adds the versioned private Life OS state document and bounded status/readout formatting. This phase is the foundation for later commands and tools and proves state stays under the existing memory STATE_DIR without entering durable prompt context.

### Changes Required:

#### 1. `packages/memory/code/extensions/memory/life-state.ts`
**File**: `packages/memory/code/extensions/memory/life-state.ts`
**Changes**: Create the private versioned Life OS schema, normalization, read/write helpers, profile/goal/reflection mutations, and reset helpers.
...
#### 2. `packages/memory/code/extensions/memory/life-text.ts`
**File**: `packages/memory/code/extensions/memory/life-text.ts`
**Changes**: Create bounded status/readout renderers for command and tool consumers, using shared UTF-8 truncation.
...
#### 3. `packages/memory/code/tests/pi-memory.test.mjs`
**File**: `packages/memory/code/tests/pi-memory.test.mjs`
**Changes**: Add phase-specific tests for private state, command routing, and tool wiring without adding forbidden command/tool surfaces.
...
## Phase 2: `/memory life ...` commands

### Overview
Adds the `/memory life ...` user command surface by delegating from the existing `/memory` router into a new small command module. This phase keeps existing memory routes intact and makes profile/goals/reflections inspectable and reversible on demand.

### Changes Required:

#### 1. `packages/memory/code/extensions/memory/life-use.ts`
**File**: `packages/memory/code/extensions/memory/life-use.ts`
**Changes**: Create the `/memory life ...` command parser and result helpers for status, readout, profile, goals, reflections, remove, and reset operations.
...
#### 2. `packages/memory/code/extensions/memory/memory-use.ts`
**File**: `packages/memory/code/extensions/memory/memory-use.ts`
**Changes**: Import Life OS command helpers, list `/memory life ...` in help, include `life` in the command description, and add one thin delegation branch.
...
```js
// Uses the Life OS state/text imports from Phase 1 plus the existing registerMemoryUse test import.
...
## Phase 3: Focused Life OS tools and docs

### Overview
Registers focused model tools for Life OS readout and narrow profile/goal/reflection updates, documents the private on-demand state rule, and adds tool-wiring tests. This terminal phase carries the full project validation commands.

### Changes Required:

#### 1. `packages/memory/code/extensions/memory/life-tools.ts`
**File**: `packages/memory/code/extensions/memory/life-tools.ts`
**Changes**: Create focused Life OS model tools for bounded readout and narrow profile/goal/reflection set/update/remove operations.
...
#### 2. `packages/memory/code/extensions/memory.ts`
**File**: `packages/memory/code/extensions/memory.ts`
**Changes**: Register Life OS tools from the memory extension without changing existing lifecycle hooks or memory tools.
...
#### 3. `packages/memory/code/extensions/memory/README.md`
**File**: `packages/memory/code/extensions/memory/README.md`
**Changes**: Document new Life OS modules plus private/on-demand storage rules and command-only reset behavior.
...
#### 4. `packages/memory/code/tests/pi-memory.test.mjs`
**File**: `packages/memory/code/tests/pi-memory.test.mjs`
**Changes**: Add phase-specific tests for private state, command routing, and tool wiring without adding forbidden command/tool surfaces.
...
### Automated
- Run the phase-specific assertions listed under each phase.
- Terminal validation is carried by Phase 3: `npm test`, `npm run pack:dry`, and `git diff --check`.
- Keep `memory_status`, `memory_search`, durable-context, and command-surface regression tests green while adding Life OS coverage.

### Manual Testing Steps
1. Verify `/memory status` still reports existing memory paths and `State dir`.
2. Verify `buildDurableMemoryContext()` does not include Life OS profile/goals/reflections by default.
3. Verify QMD search setup does not include `memory/state` or `memory/state/life`.
4. Verify no `/context`, `/journal`, `/memory query`, `/memory compact`, or new `/life` command appears.
5. Verify tools use TypeBox schemas, `truncateToolOutput()`, and `toolError()` like `memory_status`/`memory_search`.
6. Verify private JSON writes use `writePrivateJsonSync()`.
7. Run `npm test`, `npm run pack:dry`, and `git diff --check`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| code | Phase 2 §1 (life-use.ts) | <n/a> | concern | code-quality | `parseGoalAdd()` treats any lowercase first word as an explicit id, so `/memory life goal add write docs` creates id `write` and name `docs` instead of goal name `write docs` despite the stated `/memory life goal add <name>` support | Require an explicit flag or delimiter for ids, or only use the two-argument id form when documented unambiguously | applied: parser now treats only `--id` or hyphenated slug ids as explicit ids and adds a no-id multiword goal test; design and plan updated. |
| code | Phase 2 §2 (memory-use.ts) | packages/memory/code/extensions/memory/memory-use.ts:221 | concern | codebase-fit | Phase 2 creates a circular dependency because `memory-use.ts` imports `life-use.ts`, `life-use.ts` imports `life-state.ts`, and Phase 1 `life-state.ts` imports `ensureMemoryStorage` back from `memory-use.ts` | Move `ensureMemoryStorage`/directory setup into an acyclic storage helper or make `life-state.ts` depend only on `paths.ts` plus shared filesystem helpers | applied: `life-state.ts` no longer imports `ensureMemoryStorage`; reads use `lifeStatePath()` directly and writes rely on `writePrivateJsonSync()` to create private parent directories. |
| coverage | ## Verification Notes §1 | <n/a> | concern | verification-coverage | Note "Verify `/memory status` still reports existing memory paths and `State dir`." — criteria NOT FOUND, code NOT FOUND | Add a Phase 2 or Phase 3 `#### Automated Verification:` bullet asserting `/memory status` output includes existing memory paths and `State dir` | applied: added a Phase 2/Slice 2 automated success criterion for `/memory status` retaining existing paths and `State dir`. |

## Developer Context

Step 5 triage: developer chose `Apply fix` for all three Step 4 concerns. Applied parser disambiguation, removed the Life state circular dependency, and added `/memory status` success-criteria coverage in both design and plan artifacts.

## References

- Design: `.rpiv/artifacts/designs/2026-05-29_11-55-28_pi-native-life-os-personal-continuity.md`
